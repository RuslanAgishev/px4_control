...
def position_control(setpoints, mode):
    ...
    for i in range(number_of_setpoints):
	    ...
	    x,y,z = trajgen(mode=mode,x_des=x_des,y_des=y_des,z_des=z_des)
...
if __name__ == '__main__':
    try:
        ...
        if args.x and args.y and args.z is not None:
        	...
        	position_control( np.array([ [0,1.5,0] ]), 'setpoint_relative' ) # PI-letter
        	#position_control( np.array([ [0,0,0.5], [0,1,0], [0,0.7,-1.5] ]), 'setpoint_relative' )
